...
If no Rosa require exists anywhere in the project, stop instrumenting and prompt the user to
install it first — don't guess at a require path.

Check whether the project uses wally (a `wally.toml` at the repo root, or a `Packages`/
`DevPackages` folder generated by it):

- **Wally project:** present both install options, but recommend wally as the default:
  1. **(Recommended)** Add `ajsteinhauser/rosa-rbx` to `wally.toml` dependencies and run `wally
     install` — [wally package](https://wally.run/package/ajsteinhauser/rosa-rbx).
  2. Download `rosa-rbx.rbxm` from the
     [GitHub releases page](https://github.com/ajsteinhauser/rosa-rbx/releases) and drag it into
     `ReplicatedStorage` in Studio.
- **No wally:** ask a single simple yes/no question — "Rosa isn't installed. Want me to install
  the `rosa-rbx` module from source?" — then fetch the latest release from the
  [GitHub releases page](https://github.com/ajsteinhauser/rosa-rbx/releases) if they agree. Don't
  mention wally as an option here since the project isn't set up for it.

Wait for the user's answer before doing anything else — don't install and don't proceed to
instrumentation until Rosa is actually present.
